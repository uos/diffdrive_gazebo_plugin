...
void GazeboRosDiffdrive::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf )
{
  ...
  std::vector<physics::JointPtr> joints_tmp;
  ...
  for(size_t i=0; i<joint_names.size(); i++)
  {
    ...
    // get all joint objects, some of them could not exist in the model
  ...
  if(not joints_tmp[LF]) missing_joints.push_back(LF);
  if(not joints_tmp[LR]) missing_joints.push_back(LR);
  if(not joints_tmp[RF]) missing_joints.push_back(RF);
  if(not joints_tmp[RR]) missing_joints.push_back(RR);

  // if one of the middle joints is available than the other should be available, too.
  if( ((bool)joints_tmp[LM]) xor ((bool)joints_tmp[RM]))
  {
  ...
  size_t num_joints = six_wheeled ? 6 : 4;
  std::vector<int>six_wheels {LF, LM, LR, RF, RM, RR};
  std::vector<int>four_wheels {LF, LR, RF, RR};
  std::vector<int>wheels = six_wheeled ? six_wheels : four_wheels;
